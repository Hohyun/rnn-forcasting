...
-- data ------------------------------------------------------
-- Sunspot: 1~221 train, 222~288 test, total 289
...
--model:add(nn.SplitTable(1))  -- Tensor --> Sequence of table
model:add(nn.Sequencer(nn.FastLSTM(1,100)))
-- model:add(nn.DropOut(0.50))
-- model:add(nn.Sequencer(nn.FastLSTM(100,100)))
...
-- Train -------------------------------------------------
best_mae = 100
maxWait = 100   -- first minimum tries
maxTries = 100  -- wait until find better loss
